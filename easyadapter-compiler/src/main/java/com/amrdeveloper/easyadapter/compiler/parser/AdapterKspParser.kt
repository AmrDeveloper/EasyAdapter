...
class AdapterKspParser(private val logger: KSPLogger) {
    ...
    fun parseAdapterBindingList(classDeclaration: KSClassDeclaration): List<BindingData> {
        ...
        classDeclaration.getDeclaredProperties().forEach {
            ...
            if (it.isAnnotationPresent(BindLottieRaw::class)) {
                if (elementType != "Int") {
                    logger.error("@BindLottieRaw can used only with integer data type", it)
                }
                val annotation = it.getAnnotationsByType(BindLottieRaw::class).first()
                val binding = BindLottieRawData(elementName, annotation.viewId, annotation.condition)
                viewBindingDataList.add(binding)
            }

            if (it.isAnnotationPresent(BindLottieUrl::class)) {
                if (elementType != "String") {
                    logger.error("@BindLottieUrl can used only with string data type", it)
                }
                val annotation = it.getAnnotationsByType(BindLottieUrl::class).first()
